...
public class ControllerDynamicRL {
    ...
    // @io.github.resilience4j.ratelimiter.annotation.RateLimiter(name = "customRateLimiter", fallbackMethod = "fallbackRateLimiter")
    @GetMapping("/hello1")
...
class RateLimiterConfiguration {
    ...
    public RateLimiter rateLimitWithCustomConfig() {
        RateLimiterConfig customConfig = RateLimiterConfig.custom()
                ...
                // if a request exceeds 5s, trigger fallback. request will fail after 5s if it didnt get a chance to execute.
                // mengontrol brp lama harus menunggu
                .timeoutDuration(Duration.of(5, ChronoUnit.SECONDS))
                .build();

        return rateLimiterRegistry.rateLimiter("customRateLimiter", customConfig);
    }
}*/
